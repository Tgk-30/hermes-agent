...
| [`pre_api_request`](/docs/user-guide/features/hooks#pre_api_request) | Before each HTTP request to the LLM provider | `**kwargs` with request metadata such as `task_id`, `session_id`, `provider`, `model`, `base_url`, `api_call_count`, `message_count`, and size estimates | ignored |
| [`post_api_request`](/docs/user-guide/features/hooks#post_api_request) | After each HTTP response from the LLM provider | `**kwargs` with response metadata such as `task_id`, `session_id`, `provider`, `model`, `base_url`, `api_call_count`, `api_duration`, `finish_reason`, and `usage` | ignored |
